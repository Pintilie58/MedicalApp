...
// The progress tracker shares its state through IDistributedCache (SQL table in
// production) so a poll answered by another instance still sees the progress.
...
// =================================================================
// 14. Afișare progresivă: secțiunile ajung pe ecran ÎNAINTE de final
// =================================================================
{
    fakeAi.Mode = FakeAi.Behaviour.Ok;
    fakeEmail.Sent.Clear();
    ...
    var id = await NewPendingRow("u@test.ro");
    var runTask = Run(Job(id, "tok-prog"));

    // Pipeline-ul e blocat la pasul de email: exact ce vede userul la ~2/3 din drum.
    await Task.WhenAny(fakeEmail.Reached.Task, Task.Delay(15_000));
    Check("Progresiv: pipeline-ul a ajuns la pasul de email (test valid)",
        fakeEmail.Reached.Task.IsCompleted);

    var mid = tracker.Get("tok-prog");
    Check("Progresiv: jobul NU s-a terminat inca", mid?.Stage != "done", mid?.Stage ?? "null");
    Check("Progresiv: datele pacientului sunt deja pe ecran",
        mid?.Patient?.Name == "Ion Popescu" && mid?.Patient?.Sex == "M",
        mid?.Patient?.Name ?? "null");
    Check("Progresiv: laboratorul si data recoltarii sunt publicate",
        mid?.Patient?.Laboratory == "Lab Probe" && mid?.Patient?.DateTaken == "10/06/2026");
    Check("Progresiv: tabelul analizelor e deja publicat (4 analize)",
        mid?.Table?.Count == 4, (mid?.Table?.Count ?? 0).ToString());
    Check("Progresiv: valorile in afara normalului sunt numarate (low+high+borderline=3)",
        mid?.OutOfRangeCount == 3, (mid?.OutOfRangeCount ?? 0).ToString());
    Check("Progresiv: rezumatul e publicat", mid?.Summary == "sumar", mid?.Summary ?? "null");
    Check("Progresiv: recomandarile sunt publicate",
        mid?.Recommendations == "bea apa si repeta analizele", mid?.Recommendations ?? "null");
    Check("Progresiv: lista de valori anormale e completata local (completer-ul)",
        (mid?.Findings?.Count ?? 0) >= 3, (mid?.Findings?.Count ?? 0).ToString());
    Check("Progresiv: nicio eroare pe parcurs", string.IsNullOrEmpty(mid?.Error));

    // O alta instanta (alt pod) raspunde la poll: trebuie sa vada acelasi progres.
    var otherInstance = new InterpretationProgressTracker(
        sp.GetRequiredService<Microsoft.Extensions.Caching.Distributed.IDistributedCache>(),
        sp.GetRequiredService<ILoggerFactory>().CreateLogger<InterpretationProgressTracker>());
    var shared = otherInstance.Get("tok-prog");
    Check("Progresiv (SaaS): alta instanta vede aceleasi secțiuni",
        shared?.Patient?.Name == "Ion Popescu" && shared?.Table?.Count == 4
        && shared?.Recommendations == "bea apa si repeta analizele",
        shared?.Patient?.Name ?? "null");

    gate.SetResult();
    await runTask;
    fakeEmail.Gate = null;

    var end = tracker.Get("tok-prog");
    var after = await RowState(id);
    Check("Progresiv: la final jobul e success si stage=done",
        after.status == "success" && end?.Stage == "done", $"{after.status}/{end?.Stage}");
    Check("Progresiv: secțiunile ramân disponibile la final",
        end?.Patient?.Name == "Ion Popescu" && end?.Table?.Count == 4
        && end?.Recommendations == "bea apa si repeta analizele");
    Check("Progresiv: redirectul spre raport e setat", !string.IsNullOrEmpty(end?.RedirectUrl));
}
...
sealed class FakeAi : IMedicalInterpretationProvider
{
    ...
    private (InterpretationResult, int, int, string) Build()
    {
        ...
        var r = new InterpretationResult
        {
            ...
            Summary = "sumar",
            Recommendations = "bea apa si repeta analizele",
            PatientInfo = Mode == Behaviour.NotMedical ? null : new PatientInfo
            {
                Name = "Ion Popescu", Age = "52", Sex = "M",
                DateTaken = "10/06/2026", Laboratory = "Lab Probe"
            },
...
sealed class FakeEmail : IEmailService
{
    ...
    /// <summary>When set, the email step blocks until released — lets the probe
    /// look at what the upload screen would show WHILE the job still runs.</summary>
    ...
    public async Task SendEmailWithAttachmentsAsync(string toEmail, string subject, string htmlBody,
        IEnumerable<(byte[] Bytes, string FileName, string MimeType)> attachments)
    {
        Sent.Add((toEmail, subject, attachments.Count()));
        if (Gate != null) { Reached.TrySetResult(); await Gate.Task; }
    }
}
